...
const CategoryIcon = ({ label }: { label: string }) => {
  if (label === "도서/문구")
    return (
      <FaBook className="-mr-1 h-5 w-5 text-gray-800" aria-hidden="true" />
    );
  if (label === "리빙/생활")
    return (
  ...
  if (label === "패션의류")
    return (
      <IoShirtSharp
        className="-mr-1 h-5 w-5 text-green-800"
        aria-hidden="true"
      />
    );
  if (label === "패션잡화")
  ...
  if (label === "모바일/태블릿")
    return (
      <ImMobile className="-mr-1 h-5 w-5 text-indigo-400" aria-hidden="true" />
    );
  if (label === "pc/노트북")
    return (
      <MdOutlineComputer
